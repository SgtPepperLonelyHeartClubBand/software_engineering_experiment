...
>
> 开发实施以 `docs/BACKEND_A_DEVELOPMENT_SPEC.md` 为依据。团队分工表中早期写法提到 Python Flask，但当前仓库已基于 Spring Boot 完成后端主体；为保证代码、测试与报告一致，本文档按实际 Spring Boot 技术栈描述。
...
默认开发和测试环境开启该能力，`mysql` profile 中关闭该能力。`JwtAuthFilter` 的识别顺序为：优先解析 `Authorization: Bearer <token>`；只有在没有有效 JWT 且 `dev-auth-enabled=true` 时，才读取 `X-Dev-User-Id`。因此正式运行时可以通过关闭配置确保开发期 Header 不再生效。
...
| 用户识别 | JWT 和开启配置后的 `X-Dev-User-Id` 均能识别当前用户；关闭配置后开发期 Header 不生效 |
